...
class BTSyncSetup3(IAccessible):
	"""Custom class for the third BTSync setup screen"""
	...
	def script_leftArrowPressed(self, gesture):
		...
		try:
			if self.windowControlID==self.nextButton or self.windowControlID==self.copyButton:
				return
	...
	def script_rightArrowPressed(self, gesture):
		...
		try:
			if self.windowControlID==self.backButton or self.windowControlID==self.copyButton:
				return
		except AttributeError:
			pass
		# Send the gesture through to the application.
	...
	def script_upArrowPressed(self, gesture):
		...
		if self.windowControlID in [self.skipButton, self.nextButton, self.backButton, self.copyButton]:
			return
		# Send the gesture through to the application.
		gesture.send()
	...
	def script_shiftTabPressed(self, gesture):
		...
		try:
			...
			elif self.windowControlID==self.copyButton:
	...
	def script_tabPressed(self, gesture):
		...
		try:
			if self.windowControlID == self.ThisIsASecret:
